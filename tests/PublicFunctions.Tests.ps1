...
Describe -Tag "General" -Name "Properties" {
    ForEach ($application in $Applications) {
        ...
        Context "Validate $($application.Name) properties" {

            # Run each command and capture output in a variable
            $Output = Get-EvergreenApp -Name $application.Name
            
            # Test that the function returns something
            It "$($application.Name): Fuction returns something" {
                ($Output | Measure-Object).Count | Should -BeGreaterThan 0
            }

            # Test that the function output matches OutputType in the function
            It "$($application.Name): Function returns the expected output type" {
                $Output | Should -BeOfType ((Get-Command -Name $application.Name).OutputType.Type.Name)
            }
            ...
            If ([bool]($Output[0].PSObject.Properties.name -match "Version")) {
                ForEach ($object in $Output) {
                    If ($object.Version.Length -gt 0) {
                        It "$($application.Name): [$($object.Version)] is a valid version number" {
            ...
            Else {
                Write-Host -ForegroundColor Yellow "`t$($application.Name) does not have a Version property."
            }
            ...
            If ([bool]($Output[0].PSObject.Properties.name -match "URI")) {
                ForEach ($object in $Output) {
                    It "$($application.Name): URI property is a valid URL" {
                        $object.URI | Should -Match $MatchUrl
                    }
                }
            }
            Else {
                Write-Host -ForegroundColor "Yellow" "`t$($application.Name) does not have a URI property."
            }
        }
    }
}
...
Describe -Tag "Download" -Name "Downloads" {
    ForEach ($application in $applications) {
        ...
        Context "Validate $($application.Name) downloads" {
            ...
            If ([bool]($Output[0].PSObject.Properties.name -match "URI")) {
                ForEach ($object in $Output) {
                    It "$($application.Name): [$(Split-Path -Path $object.URI -Leaf)] is a valid download target" {
                        try {
                            ...
                            try {
                                ...
                                Write-Host -ForegroundColor Yellow "`t$($application.Name) requires manual testing."
                                $r = [PSCustomObject] @{
                                    StatusCode = 200
                                }
                            } #>
